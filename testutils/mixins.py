...
class GNRC:
    ...
    def add_nib_route(self, iface, route, ip_addr):
        self.pexpect.sendline("nib route add {} {} {}".format(iface, route,
                                                              ip_addr))
    ...
    def ping(self, count, dest_addr, payload_size, delay):
        self.pexpect.sendline("ping6 {} {} {} {}".format(count, dest_addr,
                                                         payload_size, delay))
        packet_loss = None
        for i in range(count+1):
            exp = self.pexpect.expect(["bytes from", "([\\d]+) packets " +
                                       "transmitted, ([\\d]+) received, " +
